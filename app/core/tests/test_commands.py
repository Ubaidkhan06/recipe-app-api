...
class CommandTests(SimpleTestCase):
    ...
    @patch('time.sleep')
    def test_wait_for_db_delay(self, patched_sleep, pathched_check):
        """Test waiting for database when getting operational error"""
        pathched_check.side_effect = [Psycopg2Error] * 2 + [OperationalError] * 3 + [True]
        call_command('wait_for_db')
        self.assertEqual(pathched_check.call_count, 6)
        pathched_check.assert_called_with(databases=['default'])
